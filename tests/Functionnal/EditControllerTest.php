...
class EditControllerTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testSingleLayoutScenario()
    {
        ...
        // We are in test mode
        $controller->toggleTestMode(true);
...
EOT
        ...
        $this->assertResponseOk($controller->addAction($request, $context, $editToken, $layout, 0, 'a', 12, 1), <<<EOT
<item id="6"/>
EOT
        );
        $this->assertResponseOk($controller->addAction($request, $context, $editToken, $layout, 0, 'b', 7, 2, 'bar'), <<<EOT
<item id="7" style="bar"/>
EOT
        );
...
EOT
        ...
        $this->assertResponseOk($controller->addColumnContainerAction($request, $context, $editToken, $layout, $c12->getStorageId(), 0, 2), <<<EOT
<horizontal id="8">
  <column id="9"></column>
  <column id="10"></column>
</horizontal>
...
EOT
        ...
        // Append
        $this->assertResponseOk($controller->addAction($request, $context, $editToken, $layout, $c22->getStorageId(), 'a', 5, 0), <<<EOT
<item id="11"/>
EOT
        );

        // Prepend
        $this->assertResponseOk($controller->addAction($request, $context, $editToken, $layout, $c22->getStorageId(), 'b', 12, 0), <<<EOT
<item id="12"/>
EOT
        );
...
EOT
        ...
        $controller->addColumnContainerAction($request, $context, $editToken, $layout, $topLevel->getStorageId(), 1, 1);
        $c3 = $topLevel->getAt(1);
        $this->assertTrue($c3 instanceof HorizontalContainer);
        $c3_2 = $c3->getColumnAt(0);
        $this->assertTrue($c3_2 instanceof ColumnContainer);

        $this->assertResponseOk($controller->addColumnAction($request, $context, $editToken, $layout, $c3->getStorageId(), 0), <<<EOT
<column id="15"></column>
EOT
        );
        ...
        $controller->addColumnAction($request, $context, $editToken, $layout, $c3->getStorageId(), 2);
...
EOT
        ...
        // Will all be removed
        $controller->addAction($request, $context, $editToken, $layout, $c3_4->getStorageId(), 'a', 54);
        $controller->addAction($request, $context, $editToken, $layout, $c3_4->getStorageId(), 'b', 32);
        $controller->addAction($request, $context, $editToken, $layout, $c3_4->getStorageId(), 'a', 12);

        // None be removed
        $controller->addAction($request, $context, $editToken, $layout, $c3_3->getStorageId(), 'b', 11, 0);
        $controller->addAction($request, $context, $editToken, $layout, $c3_3->getStorageId(), 'b', 8, 0);
        $controller->addAction($request, $context, $editToken, $layout, $c3_3->getStorageId(), 'a', 1, 2, 'foo');
...
EOT
        ...
        $this->assertResponseOk($controller->removeAction($request, $context, $editToken, $layout, $b12->getStorageId()));

        $b52 = $c3_2->getAt(1);
        $this->assertSame('52', $b52->getId());
        $this->assertSame('b', $b52->getType());
        $this->assertResponseOk($controller->removeAction($request, $context, $editToken, $layout, $b52->getStorageId()));
...
EOT
        ...
        $this->assertResponseOk($controller->moveAction($request, $context, $editToken, $layout, $c3_1->getStorageId(), $a6->getStorageId(), 0));

        // Move one item into the same container
        $b8 = $c3_3->getAt(0);
        $this->assertSame('8', $b8->getId());
        $this->assertSame('b', $b8->getType());
        $this->assertResponseOk($controller->moveAction($request, $context, $editToken, $layout, $c3_3->getStorageId(), $b8->getStorageId(), 2));
        ...
        $this->assertResponseOk($controller->moveAction($request, $context, $editToken, $layout, $c22->getStorageId(), $a1_foo->getStorageId(), 1));

        // Move a vertical container
        $this->assertResponseOk($controller->moveAction($request, $context, $editToken, $layout, $topLevel->getStorageId(), $c3->getStorageId(), 0));
...
EOT
        ...
        try {
            $controller->moveAction($request, $context, $editToken, $layout, $c3->getStorageId(), $a6->getStorageId(), 0);
            $this->fail();
        ...
        try {
            $controller->moveAction($request, $context, $editToken, $layout, $c3->getStorageId(), $c3_3->getStorageId(), 1);
            $this->fail();
        } catch (GenericError $e) {
            $this->assertTrue(true);
        }
        ...
        try {
            $controller->moveAction($request, $context, $editToken, $layout, $b8->getStorageId(), $a6->getStorageId(), 0);
            $this->fail();
        } catch (GenericError $e) {
            $this->assertTrue(true);
        }
